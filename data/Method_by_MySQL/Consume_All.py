			...
			if len(final_data)>0:
				...
				for data in final_data:
					...
					for item in dict_data['results']:
						...
						item['CONSUMETIME']=item['CONSUMETIME'].replace(':','-')
                   				try: 
							cu.execute('insert into Consume values(\''+studentID+'\',\''+item['CONSUMETYPE'].decode('gb2312')+'\',\''+item['CURRENTDBMONEY']+'\',\''+item['CARDNO']+'\',\''+item['FLOWCODE']+'\',\''+item['CONTYPE']+'\',\''+item['DISPOSETIME']+'\',\''+item['CONSUMETIME']+'\',\''+item['RN']+'\',\''+item['TRANSACTMONEY']+'\',\''+item['WINNAME'].decode('gb2312')+'\')')
